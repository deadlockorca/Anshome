...
const ANALYSIS_CATEGORY_SLUGS = analysisCategories.map((cat) => cat.slug);
...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: "Phân tích đánh giá | Anshome",
    description:
    ...
    alternates: {
      canonical: `${getSiteUrl()}/phan-tich-danh-gia`,
    },
    openGraph: {
      title: "Phân tích đánh giá | Anshome",
      description:
        "Thông tin đa dạng và chuyên sâu với những phân tích, đánh giá về thị trường BĐS dựa trên dữ liệu lớn, khảo sát thực tế và chia sẻ kiến thức của các chuyên gia.",
      url: `${getSiteUrl()}/phan-tich-danh-gia`,
      type: "website",
    },
  };
}

export default async function AnalysisHubPage({ searchParams }: Props) {
    ...
    <main className="stage-root bg-[#f5f6f8] text-[#1f2430]">
      ...
      <section className="mx-auto w-full max-w-[1200px] px-4 py-6">
        <div className="mb-4 flex items-center gap-1.5 text-[12px] font-semibold text-[#8a8f99]">
          ...
          <span>/</span>
          <span className="text-[#303743]">Phân tích đánh giá</span>
        </div>

        <h1 className="text-[22px] font-extrabold leading-8 text-[#20242d]">Phân tích đánh giá</h1>
        <p className="mt-1 text-[14px] font-semibold text-[#66707c]">
          Thông tin đa dạng và chuyên sâu với những phân tích, đánh giá về thị trường BĐS dựa trên dữ liệu lớn, khảo sát
          thực tế và chia sẻ kiến thức của các chuyên gia.
        </p>

        <div className="mb-8 mt-6 rounded-md border border-[#e1e4ea] bg-white p-4">
          <h2 className="mb-3 text-[14px] font-extrabold text-[#20242d]">Chuyên mục</h2>
          <div className="flex flex-wrap gap-3">
            {analysisCategories.map((cat) => (
              <Link
                key={cat.slug}
                href={`/phan-tich-danh-gia/${cat.slug}`}
                className="rounded-md border border-[#e1e4ea] px-4 py-2 text-[13px] font-extrabold text-[#303743] hover:border-brand hover:text-brand transition-colors"
              >
                {cat.label}
              </Link>
            ))}
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
          <section className="min-w-0">
            {featured ? (
              <Link href={`/tin-tuc/${featured.slug}`} className="group block overflow-hidden rounded-md border border-[#e1e4ea] bg-white">
                <div className="aspect-[16/9] w-full overflow-hidden bg-[#e9ecef]">
                  {featured.coverMedia?.publicUrl ? (
                    ...
                    <span className="grid h-full place-items-center text-sm font-bold text-[#7a808c]">Phân tích</span>
                  )}
                </div>
                <div className="p-5">
                  <p className="text-[12px] font-bold text-brand">{featured.category?.name ?? "Phân tích đánh giá"}</p>
            ...
            <div className="mt-6 flex items-center gap-4 border-b border-[#e1e4ea]">
              ...
              <Link
                href="/phan-tich-danh-gia?tab=hot"
                className={`pb-2 text-[13px] font-extrabold ${tab === "hot" ? "border-b-2 border-brand text-brand" : "text-[#8b8f96]"}`}
              >
                Bài viết xem nhiều
              </Link>
...
function AnalysisArticleCard({ article }: { article: ArticleCard }) {
  return (
    <article className="overflow-hidden rounded-md border border-[#e1e4ea] bg-white">
      ...
      <div className="p-4">
        <p className="text-xs font-bold uppercase tracking-normal text-brand">{article.category?.name ?? "Phân tích đánh giá"}</p>
...
function pageHref(page: number, tab: string): string {
  ...
  return `/phan-tich-danh-gia${queryString ? `?${queryString}` : ""}`;
}
