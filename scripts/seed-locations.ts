...
const MUNICIPALITY_SLUGS = new Map<number, string>([
  [1, "ha-noi"],
  [79, "tp-ho-chi-minh"],
  [48, "da-nang"],
  [31, "hai-phong"],
  [92, "can-tho"],
  [46, "thua-thien-hue"], // API returns "Thành phố Huế" but slug keeps the established "thua-thien-hue"
]);
...
const provinces: ProvSeed[] = [
  ...
  { code: 2, name: "Tỉnh Hà Giang", districts: [
    { code: 24, name: "Thành phố Hà Giang" },
    { code: 26, name: "Huyện Đồng Văn" },
    { code: 27, name: "Huyện Mèo Vạc" },
    { code: 28, name: "Huyện Yên Minh" },
    { code: 29, name: "Huyện Quản Bạ" },
    { code: 30, name: "Huyện Vị Xuyên" },
    { code: 31, name: "Huyện Bắc Mê" },
    { code: 32, name: "Huyện Hoàng Su Phì" },
    { code: 33, name: "Huyện Xín Mần" },
    { code: 34, name: "Huyện Bắc Quang" },
    { code: 35, name: "Huyện Quang Bình" },
  ] },
  { code: 4, name: "Tỉnh Cao Bằng", districts: [
    ...
    { code: 49, name: "Huyện Quảng Hòa" },
    { code: 51, name: "Huyện Hoà An" },
    { code: 52, name: "Huyện Nguyên Bình" },
    { code: 53, name: "Huyện Thạch An" },
  ] },
  { code: 6, name: "Tỉnh Bắc Kạn", districts: [
    { code: 58, name: "Thành Phố Bắc Kạn" },
    { code: 60, name: "Huyện Pác Nặm" },
    { code: 61, name: "Huyện Ba Bể" },
    { code: 62, name: "Huyện Ngân Sơn" },
    { code: 63, name: "Huyện Bạch Thông" },
    { code: 64, name: "Huyện Chợ Đồn" },
    { code: 65, name: "Huyện Chợ Mới" },
    { code: 66, name: "Huyện Na Rì" },
  ] },
  { code: 8, name: "Tỉnh Tuyên Quang", districts: [
    { code: 70, name: "Thành phố Tuyên Quang" },
    { code: 71, name: "Huyện Lâm Bình" },
    { code: 72, name: "Huyện Na Hang" },
    { code: 73, name: "Huyện Chiêm Hóa" },
    { code: 74, name: "Huyện Hàm Yên" },
    { code: 75, name: "Huyện Yên Sơn" },
    { code: 76, name: "Huyện Sơn Dương" },
  ] },
  { code: 10, name: "Tỉnh Lào Cai", districts: [
    { code: 80, name: "Thành phố Lào Cai" },
    { code: 82, name: "Huyện Bát Xát" },
    { code: 83, name: "Huyện Mường Khương" },
    { code: 84, name: "Huyện Si Ma Cai" },
    { code: 85, name: "Huyện Bắc Hà" },
    { code: 86, name: "Huyện Bảo Thắng" },
    { code: 87, name: "Huyện Bảo Yên" },
    { code: 88, name: "Thị xã Sa Pa" },
    { code: 89, name: "Huyện Văn Bàn" },
  ] },
  ...
  { code: 46, name: "Thành phố Huế", districts: [
...
function provinceCore(name: string): string {
  return name.replace(PROVINCE_PREFIX_RE, "");
}

function provinceShortName(name: string, code: number): string {
  if (code === HCM_CODE) return "TP. Hồ Chí Minh";
  return provinceCore(name);
}

function provinceSlug(name: string, code: number): string {
  const m = MUNICIPALITY_SLUGS.get(code);
  if (m) return m;
  return slugify(provinceCore(name));
}

function districtCore(name: string): string {
  return name.replace(DISTRICT_PREFIX_RE, "");
}
...
function districtName(name: string): string {
  const core = districtCore(name);
  return isNumericOnly(core) ? name : core;
}

function districtFullName(name: string, provShort: string): string {
  if (/^Thành phố\s+/i.test(name)) {
    return `${districtCore(name)}, ${provShort}`;
  }
  return `${name}, ${provShort}`;
}
...
async function main() {
  ...
  const country = await prisma.location.upsert({
    where: { type_slug: { type: "country", slug: "viet-nam" } },
    create: { type: "country", name: "Việt Nam", slug: "viet-nam", fullName: "Việt Nam", code: "VN", isActive: true },
    update: { name: "Việt Nam", fullName: "Việt Nam", code: "VN", isActive: true },
  });

  // 2. Provinces
  let provCreated = 0;
  let provUpdated = 0;
  const provSlugMap = new Map<number, string>();

  for (const p of provinces) {
    const shortName = provinceShortName(p.name, p.code);
    const slug = provinceSlug(p.name, p.code);
    const fullName = p.name;
    const codeStr = String(p.code).padStart(2, "0");

    provSlugMap.set(p.code, slug);
    ...
    if (existing) {
      ...
      provUpdated++;
      provinceIdMap.set(slug, existing.id);
    } else {
      ...
      provCreated++;
      provinceIdMap.set(slug, created.id);
    }
  }

  console.log(`Provinces: ${provCreated} created, ${provUpdated} updated (total ${provinceIdMap.size})`);

  // 3. Districts (first pass: collect all preliminary slugs to detect collisions)
  interface DistEntry {
    districtName: string;
    provCode: number;
    provSlug: string;
  ...
  const allDistricts: DistEntry[] = [];
  for (const p of provinces) {
    const pSlug = provSlugMap.get(p.code)!;
    for (const d of p.districts) {
      allDistricts.push({ districtName: d.name, provCode: p.code, provSlug: pSlug, distCode: d.code });
    }
  }

  // Pre-compute all slugs
  const slugFreq = new Map<string, number>();
  const slugMap = new Map<string, DistEntry[]>();
  for (const entry of allDistricts) {
    const core = districtCore(entry.districtName);
    let base = slugify(core);
    if (isNumericOnly(core)) base = `quan-${core}`;
    slugFreq.set(base, (slugFreq.get(base) || 0) + 1);
    if (!slugMap.has(base)) slugMap.set(base, []);
    slugMap.get(base)!.push(entry);
  }

  const finalSlugForEntry = new Map<DistEntry, string>();
  for (const [base, entries] of slugMap) {
    if (entries.length === 1) {
      finalSlugForEntry.set(entries[0], base);
      continue;
    }
    // Collision group
    const byProv = new Map<number, DistEntry[]>();
    for (const e of entries) {
      if (!byProv.has(e.provCode)) byProv.set(e.provCode, []);
      byProv.get(e.provCode)!.push(e);
    }
    for (const [, provEntries] of byProv) {
      if (provEntries.length === 1) {
        // Cross-province: suffix with province slug
        finalSlugForEntry.set(provEntries[0], `${base}-${provEntries[0].provSlug}`);
      } else {
        // Same-province: use full official name slugified
        for (const e of provEntries) {
          const full = slugify(e.districtName);
          const taken = new Set(provEntries.filter(ee => ee !== e).map(ee => finalSlugForEntry.get(ee)));
          const candidate = taken.has(full) ? `${full}-${e.provSlug}` : full;
          finalSlugForEntry.set(e, candidate);
        }
      }
    }
  }
  ...
  let distUpdated = 0;
  for (const entry of allDistricts) {
    const slug = finalSlugForEntry.get(entry)!;
    const pSlug = entry.provSlug;
    const parentId = provinceIdMap.get(pSlug);
    if (!parentId) {
      console.warn(`  Skipping district ${entry.districtName}: province ${pSlug} not found`);
      continue;
    }
    const shortName = districtName(entry.districtName);
    const provShort = provinceShortName(
      provinces.find(p => p.code === entry.provCode)!.name,
      entry.provCode,
    );
    const fullName = districtFullName(entry.districtName, provShort);
    const codeStr = String(entry.distCode);
  ...
  console.log(`Districts: ${distCreated} created, ${distUpdated} updated (total ${allDistricts.length})`);

  const totalProv = await prisma.location.count({ where: { type: "province" } });
  const totalDist = await prisma.location.count({ where: { type: "district" } });
  console.log(`Total locations: ${totalProv} provinces, ${totalDist} districts`);
}
